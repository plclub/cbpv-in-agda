...
Lemma congr_Lam { mValue : nat } { s0 : Exp  ((S) mValue) } { t0 : Exp  ((S) mValue) } (H1 : s0 = t0) : Lam (mValue) s0 = Lam (mValue) t0 .
Proof. congruence. Qed.
...
Lemma congr_App { mValue : nat } { s0 : Exp  (mValue) } { s1 : Exp  (mValue) } { t0 : Exp  (mValue) } { t1 : Exp  (mValue) } (H1 : s0 = t0) (H2 : s1 = t1) : App (mValue) s0 s1 = App (mValue) t0 t1 .
Proof. congruence. Qed.

Lemma congr_CaseS { mValue : nat } { s0 : Exp  (mValue) } { s1 : Exp  ((S) mValue) } { s2 : Exp  ((S) mValue) } { t0 : Exp  (mValue) } { t1 : Exp  ((S) mValue) } { t2 : Exp  ((S) mValue) } (H1 : s0 = t0) (H2 : s1 = t1) (H3 : s2 = t2) : CaseS (mValue) s0 s1 s2 = CaseS (mValue) t0 t1 t2 .
Proof. congruence. Qed.

Lemma congr_CaseP { mValue : nat } { s0 : Exp  (mValue) } { s1 : Exp  ((S) ((S) mValue)) } { t0 : Exp  (mValue) } { t1 : Exp  ((S) ((S) mValue)) } (H1 : s0 = t0) (H2 : s1 = t1) : CaseP (mValue) s0 s1 = CaseP (mValue) t0 t1 .
Proof. congruence. Qed.
...
Fixpoint compSubstRen_Value { kValue : nat } { lValue : nat } { mValue : nat } (sigmaValue : (fin) (mValue) -> Value (kValue)) (zetaValue : (fin) (kValue) -> (fin) (lValue)) (thetaValue : (fin) (mValue) -> Value (lValue)) (EqValue : forall x, ((funcomp) (ren_Value zetaValue) sigmaValue) x = thetaValue x) (s : Value (mValue)) : ren_Value zetaValue (subst_Value sigmaValue s) = subst_Value thetaValue s :=
    ...
    | var_Value (_) s => EqValue s
    | One (_)  => congr_One 
    | Lam (_) s0 => congr_Lam ((compSubstRen_Exp (up_Value_Value sigmaValue) (upRen_Value_Value zetaValue) (up_Value_Value thetaValue) (up_subst_ren_Value_Value (_) (_) (_) EqValue)) s0)
 ...
 with compSubstRen_Exp { kValue : nat } { lValue : nat } { mValue : nat } (sigmaValue : (fin) (mValue) -> Value (kValue)) (zetaValue : (fin) (kValue) -> (fin) (lValue)) (thetaValue : (fin) (mValue) -> Value (lValue)) (EqValue : forall x, ((funcomp) (ren_Value zetaValue) sigmaValue) x = thetaValue x) (s : Exp (mValue)) : ren_Exp zetaValue (subst_Exp sigmaValue s) = subst_Exp thetaValue s :=
    ...
    | CaseP (_) s0 s1 => congr_CaseP ((compSubstRen_Exp sigmaValue zetaValue thetaValue EqValue) s0) ((compSubstRen_Exp (up_Value_Value (up_Value_Value sigmaValue)) (upRen_Value_Value (upRen_Value_Value zetaValue)) (up_Value_Value (up_Value_Value thetaValue)) (up_subst_ren_Value_Value (_) (_) (_) (up_subst_ren_Value_Value (_) (_) (_) EqValue))) s1)
    end.
...
Lemma rinstId_Exp { mValue : nat } : @ren_Exp (mValue) (mValue) (id) = id .
...
Proof. exact (compSubstRen_Exp sigmaValue zetaValue (_) (fun n => eq_refl) s). Qed.
...
Global Instance Subst_Exp { mValue : nat } { nValue : nat } : Subst1 ((fin) (mValue) -> Value (nValue)) (Exp (mValue)) (Exp (nValue)) := @subst_Exp (mValue) (nValue) .
...
Global Instance Ren_Exp { mValue : nat } { nValue : nat } : Ren1 ((fin) (mValue) -> (fin) (nValue)) (Exp (mValue)) (Exp (nValue)) := @ren_Exp (mValue) (nValue) .
...
Notation "s [ sigmaValue ]" := (subst_Exp sigmaValue s) (at level 7, left associativity, only printing) : subst_scope.

Notation "[ sigmaValue ]" := (subst_Exp sigmaValue) (at level 1, left associativity, only printing) : fscope.

Notation "s ⟨ xiValue ⟩" := (ren_Exp xiValue s) (at level 7, left associativity, only printing) : subst_scope.

Notation "⟨ xiValue ⟩" := (ren_Exp xiValue) (at level 1, left associativity, only printing) : fscope.

Ltac auto_unfold := repeat unfold subst1,  subst2,  Subst1,  Subst2,  ids,  ren1,  ren2,  Ren1,  Ren2,  Subst_Value,  Subst_Exp,  Ren_Value,  Ren_Exp,  VarInstance_Value.

Tactic Notation "auto_unfold" "in" "*" := repeat unfold subst1,  subst2,  Subst1,  Subst2,  ids,  ren1,  ren2,  Ren1,  Ren2,  Subst_Value,  Subst_Exp,  Ren_Value,  Ren_Exp,  VarInstance_Value in *.

Ltac asimpl' := repeat first [progress rewrite ?instId_Value| progress rewrite ?compComp_Value| progress rewrite ?compComp'_Value| progress rewrite ?instId_Exp| progress rewrite ?compComp_Exp| progress rewrite ?compComp'_Exp| progress rewrite ?rinstId_Value| progress rewrite ?compRen_Value| progress rewrite ?compRen'_Value| progress rewrite ?renComp_Value| progress rewrite ?renComp'_Value| progress rewrite ?renRen_Value| progress rewrite ?renRen'_Value| progress rewrite ?rinstId_Exp| progress rewrite ?compRen_Exp| progress rewrite ?compRen'_Exp| progress rewrite ?renComp_Exp| progress rewrite ?renComp'_Exp| progress rewrite ?renRen_Exp| progress rewrite ?renRen'_Exp| progress rewrite ?varL_Value| progress rewrite ?varLRen_Value| progress (unfold up_ren, upRen_Value_Value, up_Value_Value)| progress (cbn [subst_Value subst_Exp ren_Value ren_Exp])| fsimpl].
...
Tactic Notation "asimpl" "in" "*" := auto_unfold in *; repeat first [progress rewrite ?instId_Value in *| progress rewrite ?compComp_Value in *| progress rewrite ?compComp'_Value in *| progress rewrite ?instId_Exp in *| progress rewrite ?compComp_Exp in *| progress rewrite ?compComp'_Exp in *| progress rewrite ?rinstId_Value in *| progress rewrite ?compRen_Value in *| progress rewrite ?compRen'_Value in *| progress rewrite ?renComp_Value in *| progress rewrite ?renComp'_Value in *| progress rewrite ?renRen_Value in *| progress rewrite ?renRen'_Value in *| progress rewrite ?rinstId_Exp in *| progress rewrite ?compRen_Exp in *| progress rewrite ?compRen'_Exp in *| progress rewrite ?renComp_Exp in *| progress rewrite ?renComp'_Exp in *| progress rewrite ?renRen_Exp in *| progress rewrite ?renRen'_Exp in *| progress rewrite ?varL_Value in *| progress rewrite ?varLRen_Value in *| progress (unfold up_ren, upRen_Value_Value, up_Value_Value in *)| progress (cbn [subst_Value subst_Exp ren_Value ren_Exp] in *)| fsimpl in *].

Ltac substify := auto_unfold; try repeat (erewrite rinstInst_Value); try repeat (erewrite rinstInst_Exp).

Ltac renamify := auto_unfold; try repeat (erewrite <- rinstInst_Value); try repeat (erewrite <- rinstInst_Exp).
